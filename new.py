...
while camera.isOpened():
    ...
    if isBgCaptured == 1:  # this part wont run until background captured
        ...
        # print(thresh)
        # cv2.imwrite('yo.jpg', thresh)

        oneD = thresh.flatten().tolist()
        result = model.predict([oneD])
        # print(result)
        cv2.putText(thresh, str(result[0]), (100, 100), cv2.FONT_HERSHEY_SIMPLEX, 5.0, (255, 255, 255), 5)
        ...
        # import csv
        # oneD = thresh.flatten().tolist()
        # oneD = list(map(lambda x: str(x), oneD))
        # with open('yo.csv', 'a') as c:
        #     writer = csv.writer(c, delimiter=",")
        #     writer.writerow(oneD)
